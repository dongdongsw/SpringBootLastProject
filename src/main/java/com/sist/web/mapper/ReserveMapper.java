...
//	})
	// vo.getSvo().setTitle()
	@ResultMap("resMap")
	@Select("SELECT r.no, cno, rday, rtime, rinwon, isReserve, iscancel, TO_CHAR(regdate,'yyyy-mm-dd') as dbday, "
	...
	@ResultMap("resMap")
	@Select("SELECT r.no, id, cno, rday, rtime, rinwon, isReserve, iscancel, TO_CHAR(regdate,'yyyy-mm-dd') as dbday, "
			+ "isReserve, title, image1, address "
			+ "FROM reserve_1 r, seoulTravel s "
			+ "WHERE r.cno = s.contentid "
			+ "ORDER BY no DESC")
